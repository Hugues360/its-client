...
use libits::client::analysis::analyzer::Analyzer;
use libits::client::analysis::pipeline;
...
impl Analyzer<GeoTopic, NoContext> for CopyCat {
    fn new(configuration: Arc<Configuration>, _context: Arc<RwLock<NoContext>>) -> Self
        where
            Self: Sized,
    {
    ...
    fn analyze(&mut self, mut packet: Packet<GeoTopic, Exchange>) -> Vec<Packet<GeoTopic, Exchange>> {
...
async fn main() {
    let matches = Command::new("ITS CopyCat client")
        ...
        .about("CopyCat example creates 3 second delayed clones of incoming messages from MQTT broker")
        .arg(
    ...
    let log_path = &configuration.get::<String>(Some("log"), "path").unwrap_or("log".to_string());
    ...
    if let Some(username) = matches.get_one::<String>("mqtt-username") {
        ...
        configuration.mqtt_options.set_credentials(
            username,
