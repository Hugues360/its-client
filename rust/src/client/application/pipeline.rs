    ...
    C: Send + Sync + 'static,
{
    ...
    for _ in 0..thread_count {
        ...
        analysis_pool.execute(move || {
            ...
            let mut analyser = A::new(configuration_clone, context_clone);
            for item in rx {
    ...
    let (publish_item_receiver, publish_monitoring_receiver, filter_handle) =
        filter_thread::<T, A, C>(configuration.clone(), analyser_receiver);
...
fn mqtt_router_dispatch_thread<T>(
    ...
    crossbeam_channel::Receiver<Packet<T, Exchange>>,
    Receiver<(Packet<T, Exchange>, Option<Cause>)>,
